...
!(function($) {
	...
	ts.addParser({
		...
		is: function(s) {
			...
			return (/(^\d{1,2}[\/\s]\d{1,2}[\/\s]\d{4})|(^\d{4}[\/\s]\d{1,2}[\/\s]\d{1,2})/).test((s || '').replace(/\s+/g," ").replace(/[\-.,]/g, "http://mottie.github.io/"));
		},
		format: function(s, table, cell, cellIndex) {
			if (s) {
				...
				s = s.replace(/\s+/g," ").replace(/[\-.,]/g, "http://mottie.github.io/"); // escaped - because JSHint in Firefox was showing it as an error
				if (format === "mmddyyyy") {
